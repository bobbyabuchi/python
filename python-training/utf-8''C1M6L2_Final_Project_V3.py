...
def calculate_frequencies(file_contents):
    ...
    for each_word in file_contents.split():
        text = ""
        for each_letter in each_word.lower():
            if each_letter not in punctuations and each_letter.isalpha():
                text += each_letter
        if each_word not in uninteresting_words:
            finalText.append(text)

    for each_word in finalText:
        if each_word not in wordCount:
            wordCount[each_word] = 0
